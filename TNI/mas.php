...
<html lang="en">
<head>
<title>~[ IDBTE4M SMS MASKING ]~</title>
...
function hajar(){
  var msisdn = document.getElementById('msisdn').value.split('\n');
  var current = 0;
  var total = msisdn.length;
  ...
  function ajax(){
    if(current < total){
      var nomer = msisdn[current];
      document.getElementById("load").innerHTML='<h5 style="color: #FFF;font-weight: bold;"><b>Sending .... ' + nomer + '</b></h5><div class="progress progress-striped active"><div class="progress-bar" style="width: 50%"></div></div>';//'<div class="ld ld-ring ld-spin-fast huge"></div>';
      document.getElementById("total").innerHTML = '<b>MSISDN Total: ' + total + '</b>';
      ...
      http.onreadystatechange=function() {
        if (http.readyState == 4){
          ...
          if(resp == "sukses"){
            ...
            gagal++;
            var myDiv = document.getElementById("gagal").innerHTML = "Failed: " + gagal;
          }
          ajax();
        }
      }
...
<div id="login">
  <center>
    <h3 style="color: #FFF;font-weight: bold;"><b>MASS SMS MASKING SENDER</b></h3>
    
    <div id="label">Msisdn List:</div>
    <textarea id="msisdn" style="height: 200px;" class="form-control" rows="3"></textarea><br/>
